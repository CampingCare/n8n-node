...
/**
 * Creates a reusable number field with customizable display options
 */
...
export const commonStrings = {
	limit: (resources: string[], operations: string[], defaultValue = 15, maxValue = 50) =>
		createNumberField(
			'limit',
			'Limit',
			`Maximum number of items to return (1-${maxValue})`,
			resources,
			operations,
			defaultValue,
			String(defaultValue),
	...
	offset: (resources: string[], operations: string[]) =>
		createNumberField(
			'offset',
			'Offset',
	...
	channelId: (resources: string[], operations: string[]) =>
		createStringField('channel_id', 'Channel ID', 'Filter by channel ID', resources, operations),
};
